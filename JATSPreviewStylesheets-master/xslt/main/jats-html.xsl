...
<!-- TAXPUB -->
  ...
  <xsl:template match="tp:taxon-treatment">
    <div class="section">
      <xsl:call-template name="named-anchor"/>
      <xsl:apply-templates select="tp:nomenclature"/>
      <xsl:apply-templates select="sec-meta"/>
      <xsl:apply-templates mode="drop-title-nomenclature"/>
    </div>
  </xsl:template>
  ...
  <xsl:template match="tp:material-citation">
    <div class="tooltip">
      <span class="tooltiptext">
        <b>Material Citation</b><br/>
          <xsl:for-each select="descendant::named-content[@content-type]">
            <span style="font-family: monospace;"><xsl:value-of select="@content-type"/></span><xsl:text>: </xsl:text><xsl:for-each select=".//text()"><xsl:value-of select="."/></xsl:for-each><br/>
          </xsl:for-each>
    </span>
  ...
  <xsl:template match="tp:nomenclature-citation">
    <span  style="background-color:#FFFFE0"><xsl:apply-templates/></span>
  </xsl:template>
  
  
  <xsl:template match="*" mode="drop-title-nomenclature">
    <xsl:apply-templates select="."/>
  </xsl:template>
  
  <xsl:template match="title | sec-meta | tp:nomenclature" mode="drop-title-nomenclature"/>
